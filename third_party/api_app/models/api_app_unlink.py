...
class ApiAppUnlink(models.Model):
    _name = 'api.app.unlink'
    _description = 'unlink API'
    ...
    allowed_user_list = fields.Many2many('res.users', 'api_app_unlink_allowed_user_list_rel',
                               string='Allowed Users')

    _sql_constraints = [
        ('api_app_unlink_unique_model_alias', 'unique(model_alias)', 'Model Alias must be unique!')
    ]
...
<p>Then Call:<br /></p>
<pre>
curl --location '{base_url}/api_app/unlink_data' \\
--header 'Content-Type: application/json' \\
--data '{filter_json}'
</pre>
    """
