...
class ApiAppWrite(models.Model):
    _name = 'api.app.write'
    _description = 'write API'
    ...
    allowed_field_list = fields.Many2many('ir.model.fields', 'api_app_write_allowed_field_list_rel',
                               string='Allowed Fields')

    allowed_user_list = fields.Many2many('res.users', 'api_app_write_allowed_user_list_rel',
                               string='Allowed Users')

    _sql_constraints = [
        ('api_app_write_unique_model_alias', 'unique(model_alias)', 'Model Alias must be unique!')
    ]
...
<p>Then Call:<br /></p>
<pre>
curl --location '{base_url}/api_app/write_data' \\
--header 'Content-Type: application/json' \\
--data '{filter_json}'
</pre>
    """