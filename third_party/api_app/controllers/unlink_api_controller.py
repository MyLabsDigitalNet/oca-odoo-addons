...
class UnlinkApiController(http.Controller):
    @http.route('/api_app/unlink_data', type='json', auth='user', csrf=False, methods=['POST'])
    def unlink_data(self, **kw):
        ...
        api_call_log_save(request,api_type = 'UNLINK')
        ...
        if update_record.create_uid.id != request.env.user.id:
            return {
                'response': 'ERROR',
                'data_received': 'You dont have access to unlink this record!'
            }

        update_record.unlink()
        return {
            'response': 'OK',
            'data_received': 'UNLINKED'
        }
